...
class InstanceManager {
  ...
  void setupInstance(
      String? instanceHostname, Map<String, dynamic>? instanceConfig) {
    if (debugInstance) {
    ...
    if (instanceConfig != null) {
      if (instanceConfig.containsKey('instance')) {
        (instanceConfig['instance'] as Map<String, dynamic>)
            .forEach((key, value) {
  ...
  Uri apiPath(String route,
      [Map<String, dynamic>? parameters,
      String method = 'GET',
      pathPrefix = 'api/']) {
    if (_instanceHost == "") {
      // setupInstance MUST be called first!
      throw Exception("Instance manager has no host setup!");
    }
    final bool useUnsecure = _instanceConfig['DEBUG_HTTP'] ?? false;
    print((pathPrefix == 'api/' ? "API" : "Asset") +
        " Request [" +
        (useUnsecure ? 'HTTP' : 'HTTPS') +
        "] " +
        route);
    return Uri(
      ...
      host: _instanceHost,
      path: pathPrefix +
          (route.substring(0, 1) == '/' ? route.substring(1) : route),
  ...
  Future<APIResponse> apiRequest(String route,
      [Map<String, dynamic>? parameters, String method = 'GET']) {
    ...
    if (method != 'GET') {
      ...
      parameters.putIfAbsent(
          '_method',
          () =>
              method); // Add laravel-specific _method handling for PUT/etc. to simulate HTTP forms.
    }
    ...
    final reqHttp = method == 'GET'
        ...
        : http.post(reqPath,
            headers: reqHeaders,
            body: (parameters is String ? parameters : jsonEncode(parameters)));
  ...
  Future<http.Response> assetRequest(String assetRoute) {
    final assetPrefix = AuthManager().isTestUser
        ? 'storage/gamedata.test/'
        : 'storage/gamedata/';
    final reqPath =
        InstanceManager().apiPath(assetRoute, {}, 'GET', assetPrefix);
    final reqHeaders = {...AuthManager().authTokenHeaders};
  ...
  Future<String> decryptTransparentAsset(String jsonStr) {
    ...
    if (jsonVersion != '1') {
      throw const FormatException(
          "Invalid or unhandled version tag in transparent encrypted asset file");
    }
    // Ciphertext, IV, etc. are all stored as JSON in the rest of the text.
    var cipherObj = jsonDecode(jsonStr.substring(13));
    if (cipherObj is! Map ||
        !cipherObj.containsKey('iv') ||
        !cipherObj.containsKey('value') ||
        !cipherObj.containsKey('mac')) {
      throw const FormatException(
          "Transparent encrypted asset file is invalid");
    }

    var aes = AesCrypt(
        key: 'n3/IxlLPUnbLvJnXpCUXi3WvTO6kK9za5DEDSKieJB0=',
        padding: PaddingAES.pkcs7);
    var plaintext =
        aes.cbc.decrypt(enc: cipherObj['value'], iv: cipherObj['iv']);
  ...
  Future<bool> reportEndpoint(
    ...
    final apiReportPromise = InstanceManager().apiRequest(
        ...
        'POST');
    return apiReportPromise.then(
        (apiReport) => apiReport.success(APIResponseJSON.map) ? true : false);
  }
  ...
  Future<bool> saveProfileData(Map<String, dynamic> newProfile) {
    ...
    final apiSavePromise = InstanceManager().apiRequest(
        "users/${AuthManager().loginUser['id']}/profile",
        {
          'data': profileJSON,
        },
        'POST');
    return apiSavePromise
        .then((apiSave) => apiSave.success(APIResponseJSON.map) ? true : false);
  }
  ...
  Future<String> loadProfileData() {
    ...
    final apiLoadPromise = InstanceManager().apiRequest(
        "users/${AuthManager().loginUser['id']}/profile", null, 'GET');
    return apiLoadPromise.then((apiLoad) {
      ...
      if (profileResult.containsKey('models') &&
          profileResult['models'] is List) {
        models.clear();
...
class APIResponse {
  ...
  late dynamic
      result; // JSON could be List<dynamic>, Map<String, dynamic>, etc.
